...
class AdminAuth
{
    ...
    private $authExcept = [
        ...
        'admin.auth.logout',
        'admin.editor.server',
        'admin.customer_addresses.list_by_customer_id',
    ];
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!in_array($url, $this->authExcept, true))
        {
            ...
            //登录后，验证有无权限
            if (!in_array(1,$loginUser["role"]) && !$this->authCheck($url,$loginUser) && !(strstr($url,'.create') || strstr($url,'.update'))) {
                error('无权限', $request->isMethod('get') ? 'admin.index.index' : URL_CURRENT);
            }
